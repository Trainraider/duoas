# duoas - doas with dual authentication

## Please note: the dual authentication feature was added by AI generated code, and you probably shouldn't use this project!

## Please note: The original doas has moved to CodeBerg. The project's new repository is located at https://codeberg.org/thejessesmith/doas/

A port of OpenBSD's `doas` which runs on FreeBSD, Linux, NetBSD, illumos,
macOS and MidnightBSD.

The `duoas` utility is a program originally written for OpenBSD (called doas there) which allows
a user to run a command as though they were another user. Typically `duoas`
is used to allow non-privileged users to run commands as though they were
the root user. The `duoas` program acts as an alternative to sudo, which is
a popular method in the Linux community for granting admin access to
specific users.

The `duoas` program offers three benefits over sudo: its configuration file
...
potentially open security holes in the system. And now you can require two passwords
...
Installing `duoas` is accomplished in three steps:

1. Installing build tools.
2. Compiling and installing the `duoas` utility.
3. Creating a configuration file for `duoas`.

## Getting Started

### 1. Installing build tools

The `duoas` program has virtually no dependencies. So long as you have a
...
To install `duoas`, download the source code and, in the source code's
...
> [!NOTE]  
> Some Linux distributions, such as CentOS, will block `duoas` from using
...
> administrator:
> ```sh
> cp /etc/pam.d/sudo /etc/pam.d/duoas
> ```
...
text editor called `/etc/pam.d/duoas` and insert the following lines:
...
To build `duoas`, you'll need to have Xcode Command Line Tools, and use
...
> [!NOTE]
> By default macOS blocks `duoas` from using PAM modules, causing `duoas`
> authentication to fail. The cp command above copies the sudo PAM
> configuration into place for `duoas` to use.
...
> `duoas`. Having these directories, like `/usr/local/bin` and
> `/usr/local/etc`, writable to your user means anyone can remove and
> replace your `duoas.conf` file or the `duoas` binary, allowing anyone or
> any program to run commands as root on your system or harvest your
> password. This is a large security hole and outside the scope of `duoas`.
...
The `duoas` configuration file is located at `/usr/local/etc/duoas.conf` or
`/opt/local/etc/duoas.conf` for illumos. To create a rule allowing a user to
perform admin actions, add a line to the configuration file. Details on how
to do this are covered on the `duoas.conf` manual page. However, most of the
...
To require two people to authenticate:

```sh
permit jack,jill as root
```

Additional users can be added to the file, one (or two) per line.

> [!NOTE]
> A shell script, `viduoas`, is included with the `duoas` program. The
> `viduoas` script must be run as the root user and will perform a syntax
> check on the `duoas.conf` file before installing it on the system. This
> avoids breaking the `duoas.conf` file. The `viduoas` script accepts no
...
Please be aware that, by default, `duoas` scrubs most environment variables.
...
run from `duoas`, please use the `keepenv` keyword in the configuration file.
See the duoas.conf manual page for details.

## Running commands using duoas

To make use of `duoas`, run it in front of any command. Here are some
examples:

Confirm `duoas` is working by printing our effective user ID:
```sh
duoas id
```
...
```sh
duoas touch /root/new-file
```

Edit a text file that requires admin access without running the text
editor as the root user:
```sh
duoasedit /path/to/text/file
```

On Linux versions of `duoas` prior to `6.3p1` required commands with
...
root:
```sh
duoas -- rm -rf old-directory
```

## Uninstalling `duoas`

To remove `duoas` and its helper programs, you can pass the "uninstall"
parameter to the `Makefile`. On most Linux distributions, you can run the
following from the `duoas` source directory:
...
developer privately at trainraider7 AT gmail DOT com. Or if the security concern is related to the original 'doas' program, contact the original developer privately at jessefrgsmith AT yahoo DOT ca.


Financial donations (to the original doas project) are always welcome and can be submitted via PayPal to
